...
| B01-ID-006 | SF-002, DEC-008 | La taille physique en centimètres et le poids en kilogrammes sont obligatoires et bornés par l'espèce. La catégorie P/M est déduite automatiquement de la taille physique. La taille physique est immuable après acceptation ; le poids reste modifiable. | Champs bornés par espèce au backend et sur l'échelle du wizard ; catégorie calculée par le backend. **Conforme**. | Créateur, système, MJ ; taille physique, poids, catégorie calculée. | Refus absent ou hors bornes ; unités et limites affichées sans ambiguïté ; catégorie calculée sans choix supplémentaire ; permissions après acceptation respectées. | DEC-003, DEC-008 ; SF-002. |
...
| B01-ESP-002 | PHB24 p. 186 | Au niveau 1, l'Aasimar reçoit la catégorie P ou M déduite de sa taille physique (B01-ESP-012), puis Résistance céleste, Vision dans le noir, Mains guérisseuses et Porteur de lumière (le sort mineur Lumière, incanté avec le Charisme), sans choix interne de Révélation céleste. Cette dernière n'arrive qu'au niveau 3 et son option est choisie à chaque activation. | Clé absente du contrat, du domaine, des données locales et de l'interface. **Manquant**. | Créateur ; espèce Aasimar, taille. | Un Aasimar niveau 1 valide reçoit sa catégorie calculée et peut être soumis sans choix prématuré de Révélation céleste ; sa fiche justifie ses quatre traits de niveau 1 et calcule DD et attaque de Lumière sur le Charisme. | Révélation céleste relève de B02 ; SF-002. |
...
| B01-ESP-009 | PHB24 p. 194, `races/human.json` | L'Humain reçoit la catégorie P ou M déduite de sa taille physique (B01-ESP-012) et choisit une compétence et un don d'Origine additionnel ; tous les paramétrages du don choisi sont complétés. | Compétence et don proposés ; catégorie de taille calculée depuis la taille physique ; choix du don non validé exhaustivement au backend. **Partiel**. | Créateur, système ; compétence, don et sous-choix ; catégorie calculée. | Un choix de chaque type ; don limité aux dons d'Origine éligibles ; choix imbriqués exacts. | DEC-008 ; SF-002. |
...
| B01-ESP-011 | PHB24 p. 196, `races/tiefling.json` | Le Tieffelin reçoit la catégorie P ou M déduite de sa taille physique (B01-ESP-012) et choisit un héritage fiélon parmi Abyssal, Chthonien ou Infernal et la caractéristique d'incantation autorisée. | Héritages et caractéristique présents ; catégorie de taille calculée depuis la taille physique. **Partiel**. | Créateur, système ; héritage, caractéristique ; catégorie calculée. | Trois choix valides et indépendants ; résistance et sorts correspondent uniquement à l'héritage. | DEC-008 ; SF-002. |
| B01-ESP-012 | PHB24 p. 186, 194 et 196, SF-002, DEC-008 | La catégorie de taille D&D est persistée séparément de la taille physique et limitée aux options de l'espèce. L'application la déduit de la taille physique : moins de 122 cm donne P pour l'Aasimar, l'Humain et le Tieffelin ; 122 cm ou plus donne M. Elle devient immuable après acceptation. | Catégorie absente des requêtes, calculée par le backend depuis la taille physique et persistée. **Conforme**. | Système ; taille physique et catégorie P/M calculée. | Aucun choix P/M à l'écran ; refus d'une mesure ou catégorie incohérente ; réouverture fidèle ; calculs de fiche et pion utilisent la catégorie calculée. | Dérogation d'interface et seuil produit validés ; DEC-008 ; SF-002. |
...
| B01-ORI-006 | PHB24 p. 200–202, DATA-DON | Façonneur choisit trois outils d'artisan ; Musicien trois instruments ; Doué trois maîtrises dans toute combinaison compétences/outils. | Les trois dons font choisir leurs maîtrises dans l'étape Dons, et le serveur valide catalogue et quota. Un outil déjà connu, fixe ou choisi par une autre source (classe, historique, autre don), est indisponible et justifié à l'écran. **Conforme**. | Créateur ; choix propres au don. | Chaque don impose son catalogue et son quota exact ; aucun doublon ne crée une seconde maîtrise ; Doué partage son quota de trois entre compétences et outils. | Catalogues primaires pointés ; SF-002. |
...
| B01-SOR-006 | SF-002 | Un sort ne se choisit qu'une fois à la création, toutes sources de choix confondues : sorts mineurs et préparés de classe, grimoire, chaque Initié à la magie et Pacte du grimoire. Un sort déjà accordé sans choix par l'espèce, la lignée, la classe ou son ordre ne se choisit pas non plus. Au-delà de la création, un même sort acquis deux fois reste régi par `B06-CAT-004`. | Le même sort pouvait être choisi par la classe et par un Initié à la magie, ou redoubler un sort d'espèce. **Corrigé**. | Créateur, système ; sorts choisis par source. | Un sort déjà choisi dans une source est indisponible dans les autres, avec sa justification ; le serveur refuse une création qui le choisit deux fois. | SF-002. |
