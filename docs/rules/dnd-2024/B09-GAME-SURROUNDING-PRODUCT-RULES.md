...
| B09-BUT-007 | SF-005 | Un participant ouvre le contenant visible d'une créature vaincue depuis son portrait ; plusieurs participants voient le même état en temps réel. | UI et temps réel absents. **Manquant**. | Joueurs participants, MJ ; projection partagée. | Un non-participant ou membre d'une autre campagne est refusé côté serveur. | SF-001. |
...
| B09-BUT-010 | SF-005 | La première acquisition valide persistée gagne ; les autres commandes concurrentes relisent l'état et échouent proprement. | Concurrence absente. **Manquant**. | Système ; version et quantité. | Deux prises du dernier exemplaire produisent un seul propriétaire et aucune quantité négative. | Phase 5 transaction. |
...
| B09-BUT-015 | SF-005 | Aucun mécanisme de besoin/cupidité, enchère, vote ou réservation temporaire n'est introduit au MVP : seule la première prise serveur valide attribue le bien. | Aucun workflow. **Manquant**. | Participants ; commande de prise. | Un clic non persisté n'accorde aucun droit futur ; les joueurs voient la disparition validée. | Cible existante. |
...
| B09-RES-004 | SF-005 | Le combat passe à `TERMINÉ` seulement après le transfert complet ; le butin clos n'est jamais rouvert. | Combat absent. **Manquant**. | Système ; transition finale. | Répéter la fermeture ne retransfère rien ; dupliquer le combat crée une nouvelle préparation sans ancien butin. | DEC-004. |
...
| B09-AUD-005 | Gouvernance, DEC-014 | Les conflits et refus de règle utiles à l'arbitrage sont conservés dans l'audit fonctionnel consultable par les MJ. Leur rétention plus courte sera définie en Phase 5 ; refus d'autorisation et traces de sécurité restent dans un journal technique distinct. | Aucun journal. **Manquant**. | Système, MJ ; refus, audience et catégorie. | Un refus ne contient pas de secret inaccessible à l'acteur ; un conflit de butin ou de version reste explicable sans exposer le journal de sécurité. | `DR-B09-07` résolue. |
...
- `DR-B09-05` — chaque dénomination monétaire est une pile récupérable partiellement ;
  la première prise serveur valide gagne et aucune bourse de groupe n'est créée.
...
- `DR-B09-07` — les conflits et refus de règle utiles aux MJ restent dans l'audit
  fonctionnel ; refus d'autorisation et traces de sécurité restent séparés.
