...
| Création niveau 1 | Toutes les combinaisons valides, fiche soumise au MJ | 72 des 85 règles B01 conformes **au niveau domaine seulement** ; le wizard ne produit pas encore les compositions correspondantes (maîtrises d'armes, outils, invocations, grimoire, second Initié à la magie). Soumission au MJ absente | Backend couvert, parcours réel incomplet |
...
| Identité de création | Alignement, âge, taille, poids, description et portrait selon la cible | Alignement, âge, taille, poids et description au contrat, exigés à la création et persistés ; portrait toujours absent | Fonctionnalité couverte sauf portrait |
| Choix d'origine | Langues, taille d'espèce, dons et outils validés | Deux langues standards exigées, catégorie de taille déduite de la taille physique, outils concrets et dons d'Origines validés par le serveur | Règles métier couvertes |
| Choix de classe niveau 1 | Tous les outils, maîtrises, expertise, invocation et grimoire requis | Bottes d'armes, style de combat, invocations, expertise et grimoire validés au domaine ; la source et les champs de chaque choix sont sur liste blanche | Fonctionnalité couverte |
| Sorts de niveau 1 | Listes, sources et quotas validés par le serveur | Listes, provenances et quotas validés par origine ; grimoire du Magicien distinct et préparés contraints à y figurer. Le corpus reste borné aux niveaux 0-1 | Autorité serveur rétablie, corpus partiel |
| Équipement de départ | Paquetages, possession, port, or et babiole gratuite cohérents | Objets et or recalculés depuis le paquetage retenu, options étrangères refusées, cent babioles disponibles ; magasin et achat hors périmètre | Règle métier couverte hors achat |
...
| Jets de création | Jets serveur, sans exception | Le serveur émet le tirage par le port `DICE`, le persiste et le consomme une seule fois ; la composition ne transporte plus que sa référence | Exception DEC-005 fermée |
| Autres jets | Serveur autoritaire, historique, audience figée avant tirage ; joueur public par défaut ou privé, MJ secret par défaut ou public | Le port de dé a son premier client, avec reçu et audit d'audience `target-user` ; aucun jet de partie, historique consultable ni projection | Fonctionnalité manquante malgré un premier usage du socle |
