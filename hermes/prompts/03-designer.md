...
2. Produis dans `docs/design/` :
   ...
   - **Spécification d'interaction** : états (vide, chargement, erreur, succès), transitions, raccourcis clavier. Chaque état visible référence l'UA de la spec qui le définit (`état erreur → UA-012, message exact de la table`) : les messages affichés sont CEUX fixés par l'analyste, jamais réinventés.
