import { isFullyAssigned } from "./wizard-draft";
import {
  cantripQuotaOf,
  classOf,
...
export function visibleSteps(context: StepContext): WizardStep[] {
...
export function isStepValid(step: WizardStep, context: StepContext): boolean {
  return STEPS.find((entry) => entry.key === step)?.isValid(context) ?? true;
}

export function stepProgress(step: WizardStep, context: StepContext): StepProgress | null {
