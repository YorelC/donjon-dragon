...
## Pipeline et Kanban
Chaque tâche vient d'un ticket `t_xxxx`, et l'ID figure dans le commit :
`type(scope): sujet [t_xxxx][UA-NNN]`. Lire le fichier `MODE` à la racine avant toute
tâche. Chaîne de production, conventions de board et dual-sandbox :
`docs/pipeline-hermes.md`.
