...
import { hashMemberPromotion } from '../campaign-intent';
...
export class PromoteCampaignMemberUseCase {
  ...
  async execute(
    ...
    const context = promotionContext(dto);
    const replay = await replayCampaignLifecycle(this.lifecycle, context);
    if (replay) return replay;
    const command = await this.prepare(context);
    const receipt = await this.lifecycle.promote(command, participant);
    return acceptedResult(receipt, context.intentHash);
  }

  private async prepare(context: PromotionContext): Promise<CampaignLifecycleCommand> {
    ...
    campaign.promote(actorId, targetId, occurredAt);
    return {
...
function promotionContext(dto: PromoteCampaignMemberDto): PromotionContext {
  return {
    dto,
    principalId: UserId.create(dto.actorId),
    idempotencyKey: dto.idempotencyKey,
    intentHash: hashMemberPromotion(dto.campaignId, dto.displayName, dto.expectedRevision),
  };
}
