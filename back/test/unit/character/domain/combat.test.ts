...
// ============================================================
// RED: ces tests échouent car combat.entity.ts / combat.ts n'existent pas
// ============================================================
...
import type {
  ...
  CombatState,
  AttackRoll,
} from '../../../../src/combat/domain/combat.entity.js';
...
describe('combat — rollInitiative()', () => {
  it('assigne une initiative = jet + mod DEX', () => {
...
describe('combat — resolveAttack()', () => {
  ...
  it('critique sur attackRoll = 20 (touche toujours)', () => {
    const attacker = baseCombatant({ id: 'attacker' });
  ...
  it('échec critique sur attackRoll = 1 (rate toujours)', () => {
    const attacker = baseCombatant({ id: 'attacker' });
    const target = baseCombatant({ id: 'target', armorClass: 1 });
    const roll: AttackRoll = {
      attackerId: 'attacker',
      targetId: 'target',
  ...
  it('avantage/désavantage sont des métadonnées : resolveAttack ne les recalcule pas', () => {
    const attacker = baseCombatant({ id: 'attacker' });
    const target = baseCombatant({ id: 'target', armorClass: 15 });
    const roll: AttackRoll = {
      attackerId: 'attacker',
...
describe('combat — rollDamage()', () => {
  it('roll normal (non critique) avec modificateur', () => {
    const result = rollDamage({ die: 'd6', count: 2, modifier: 3 }, false);
  ...
  it('critique double le nombre de dés', () => {
    const result = rollDamage({ die: 'd6', count: 2, modifier: 3 }, true);
    // Critique : 4d6 + 3
...
describe('combat — isCombatOver()', () => {
  it('retourne false si plus d\'un participant a des HP > 0', () => {
    const state: CombatState = {
      id: 'c1',
      roomId: 'r1',
      participants: [baseCombatant({ id: 'a' }), baseCombatant({ id: 'b' })],
      turnOrder: ['a', 'b'],
      currentTurnIndex: 0,
      round: 1,
      status: 'active',
  ...
  it('retourne true si au plus un participant a des HP > 0', () => {
    const state: CombatState = {
      id: 'c1',
      roomId: 'r1',
      participants: [
